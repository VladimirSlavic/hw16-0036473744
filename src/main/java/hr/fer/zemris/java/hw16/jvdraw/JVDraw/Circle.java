...
public class Circle extends GeometricalObject
{
	...
	/**
	 * Public constructor
	 * @param p1 first point that user clicked
	 * @param p2 second click point
	 */
	...
	public void drawShape(Graphics g, Color fg, Color bg) {
		...
		draw( g,  fg,  bg,(int)s.getX(), (int)s.y, radius);
		
	}
	
	/**
	 * Method for drawing a circle on canvas.
	 * @param g graphics
	...
	private void draw(Graphics g, Color fg, Color bg, int xc, int yc, int radius){
		...
		g.drawOval((int) (xc - radius), yc - radius,
				(2*radius),(2*radius));
	}
	...
	public String getFileString() {
		return String.format("CIRCLE %d %d %d %d %d %d", (int)s.getX(),(int) s.getY(), getRadius(), 
				foregroundC.getRed(), foregroundC.getGreen(), foregroundC.getBlue());
	}

	@Override
	public void drawShape(Graphics g, int xOffsett, int yOffset) {
		draw( g,  getForegroundC(),    getBackgroundC()
				,(int) (s.getX() - xOffsett), s.y - yOffset, getRadius());
